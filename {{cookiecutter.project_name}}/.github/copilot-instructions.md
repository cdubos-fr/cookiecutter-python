...
## Aperçu rapide
Projet Python packagé avec Flit, orchestré par tox + tox-uv. L’environnement local est géré en `.venv` via `tox --devenv`. Qualité: Ruff (lint/format), ty + mypy (types), pytest (tests+cov). Automatisations via Justfile et pre-commit. Docs avec Zensical (base minimale).
...
- Typage: `tox -e typing` (ty + mypy strict sur package + tests; configuration dans `pyproject.toml`).
...
- `pyproject.toml`: metadata, dependency-groups, configuration des outils (ruff, ty, mypy, commitizen).
...
Conseil: adaptez les propositions aux conventions établies (dependency-groups, tox-uv, pre-commit). Conservez la cohérence Ruff/ty/mypy et évitez toute régression de couverture (<80%).

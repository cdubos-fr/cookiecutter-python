...
## Vue d'ensemble
Template Cookiecutter pour projets Python simples (sans Nix) avec : packaging Flit, tox + tox-uv, virtualenv, dependency-groups (PDM style), lint/format (Ruff), types (ty), tests (pytest+cov), docs (Zensical) et automatisations via Justfile.
...
- `{{cookiecutter.project_name}}/` : Racine du projet généré
  ...
  - `docs/` : Contenu Zensical (`index.md` minimal)
...
- Typage : `tox -e typing` (exécute ty sur package + tests, options strictes via override dans `[tool.ty]`).
- Docs : `tox -e docs` ou `zensical build`. `serve` local : `just serve-docs`.
...
- ty : mode strict appliqué aux modules du package (overrides).
...
- Zensical = docs statiques (aucune config avancée ici, base extensible).
...
- Typage : `tox -e typing` ou `ty --check --project {{cookiecutter.project_slug}}`
- Docs build : `zensical build` ; serveur : `zensical serve`
- Release : `cz bump`

## Fichiers clés à consulter
- `pyproject.toml` : metadata + dependency-groups + config outils (ruff, ty, commitizen)
...
Adaptez les suggestions aux conventions établies (dependency-groups, tox-uv, pre-commit). En cas de nouvelle dépendance, décider si elle va dans `dependencies` (runtime) ou un groupe (`dev`/`typing`/`tests`/`docs`). Garder cohérence Ruff/ty. Signaler toute régression de couverture (<80%).
